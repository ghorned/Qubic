# Qubic
A 3D tic-tac-toe game with an unbeatable AI

## Description
This program provides the user with the opportunity to play a "challenging" game of 3D tic-tac-toe against an unbeatable AI. The gameplay takes place in the command line where a simple visual representation of the current game is displayed to the user as a 4x4x4 cube. The game begins with a simulated coin toss to decide who goes first and then the game continues with each player taking their turn until either a win or tie is reached.
```
...
## Implementation
All source code for this program was kept in a single .cpp file to make sharing and installation simple and easy. I chose to use C++ because of its speed. I wanted my AI to be able to search the game tree as quickly (and, therefore, as deeply) as possible in order to increase its performance.
...
The idea behind this is to provide the AI with incentive to place their markers in positions intersected by the most wining lines (higher potential), while also giving preference to placing more markers in a single line (pursue wins). A higher score evaluates to a better state for the AI and a lower score evaluates to a better state for the user.
